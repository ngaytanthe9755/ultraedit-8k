...
const NewCreation: React.FC<NewCreationProps> = ({ addToast, addNotification, currentUser, onRequireAuth, isAuthenticated, isGlobalProcessing, setGlobalProcessing }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
          ...
          reader.readAsDataURL(file);
          // Tự động chuyển engine sang Gemini Pro nếu có ảnh tham chiếu
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      addToast('Thành công', `Đã hoàn thành với engine ${engine}!`, 'success');
    ...
    <div className="flex flex-col lg:flex-row w-full h-full p-4 lg:p-6 gap-6 lg:gap-8">
      <div className="w-full lg:w-[420px] flex flex-col gap-6 lg:overflow-y-auto lg:pr-2 custom-scrollbar shrink-0 order-2 lg:order-1 pb-20 lg:pb-0">
        ...
        <div className="pb-4 border-b border-white/5">
          ...
          <p className="text-xs text-zinc-500 font-mono uppercase tracking-widest">Default: Gemini 3.0 Pro & Imagen 3</p>
        </div>
        ...
        <div className="bg-zinc-900/60 border border-white/5 p-4 rounded-2xl space-y-3">
            <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider flex items-center gap-2">
                <Cpu size={12} className="text-indigo-400"/> Bộ máy xử lý hình ảnh
            </label>
            <div className="flex bg-zinc-950 p-1 rounded-xl border border-zinc-800">
                <button 
                    onClick={() => setEngine('imagen')}
                    className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${engine === 'imagen' ? 'bg-indigo-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                    Imagen 3 (Mặc định)
                </button>
                <button 
                    onClick={() => setEngine('gemini-pro')}
                    className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${engine === 'gemini-pro' ? 'bg-purple-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                    Gemini 3.0 Pro (Nano)
                </button>
            </div>
            <p className="text-[9px] text-zinc-500 italic">
                * Imagen 3 tốt nhất cho ảnh mới. Gemini Pro tốt nhất cho sửa ảnh & sáng tạo từ ảnh mẫu.
            </p>
        </div>
                 ...
                 <div className="flex justify-between items-center p-2 mt-1">
                     <div className="flex gap-2">
                        <button onClick={handleGetSuggestions} disabled={isSuggesting} className="flex items-center gap-1.5 text-[10px] font-bold bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-1.5 rounded-lg transition-colors border border-white/5">
                            {isSuggesting ? <Loader2 size={12} className="animate-spin"/> : <Lightbulb size={12} className="text-yellow-500"/>} Gợi ý
                        </button>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select value={quality} onChange={(e) => setQuality(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 text-white text-xs font-bold rounded-lg p-2.5 outline-none focus:border-fuchsia-500 appearance-none">
                        {QUALITIES.map(q => <option key={q} value={q}>{q} High-Res</option>)}
                    </select>
                </div>
                <div>
                    ...
                    <div className="flex gap-1">
                        {[1, 2, 3, 4].map(n => (
                            <button key={n} onClick={() => setBatchSize(n)} className={`flex-1 py-2 rounded-lg text-xs font-bold border transition-all ${batchSize === n ? 'bg-fuchsia-900/30 border-fuchsia-500 text-fuchsia-400' : 'bg-zinc-950 border-zinc-800 text-zinc-500 hover:bg-zinc-900'}`}>{n}</button>
                        ))}
                    </div>
                </div>
            </div>
        ...
        <button 
            ...
            className="w-full py-4 rounded-xl bg-gradient-to-r from-indigo-600 via-fuchsia-600 to-pink-600 bg-[length:200%_auto] hover:bg-right transition-all duration-500 text-white font-bold disabled:opacity-50 flex items-center justify-center gap-3 shadow-lg group border border-white/10"
        >
            {isGenerating ? <Loader2 size={18} className="animate-spin" /> : <><Wand2 size={18}/> Generate {engine === 'imagen' ? 'Imagen 3' : 'Gemini Pro'}</>}
        </button>
      </div>
          ...
          <div className="flex-1 bg-zinc-900/30 border border-white/5 rounded-3xl relative overflow-hidden backdrop-blur-sm flex items-center justify-center group/main">
                {selectedPreview ? (
                    <div className="relative w-full h-full p-4 md:p-8 flex items-center justify-center animate-in fade-in zoom-in duration-300">
                        <img src={selectedPreview} className="max-w-full max-h-full object-contain rounded-lg shadow-2xl shadow-black/50" />
      ...
      <SuggestionModal isOpen={showSuggestionsModal} onClose={() => setShowSuggestionsModal(false)} title="Gợi ý từ Gemini 3.0 Pro" suggestions={suggestions} onSelect={(item) => { setPrompt(item.en); setShowSuggestionsModal(false); }} isLoading={isSuggesting}/>
    </div>
  );
};
